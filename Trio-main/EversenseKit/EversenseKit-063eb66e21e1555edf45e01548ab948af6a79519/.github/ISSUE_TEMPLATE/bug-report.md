---
name: "\U0001F41B Bug report"
about: Create a report to help us fix things
title: ''
type: "bug"
assignees: 'bastiaanv'

---
## Describe the bug
*A clear and concise description of what the bug is. Describe what you see versus what you expect to see.*

## Attach a Log
*Tap the Medtrum menu in your OSAID app, then scroll down and tap 'Share Medtrum logs' and attach it to this ticket.*

## To Reproduce
*Steps to reproduce the behavior:*
1. *Go to '...'*
2. *Click on '....'*
3. *Scroll down to '....'*
4. *See error*

## Expected behavior
*A clear and concise description of what you expected to happen.*

## Screenshots
*If applicable, add screenshots to help explain your problem.*

## Setup Information (please complete the following information):

### Smartphone:
* Hardware: *[e.g. iPhone 15 Pro]*
* OS Version: *[e.g. iOS 17.5]*

### CGM:
* Device: Senseonics Eversense
* Transmitter version *[e.g. 365 or E3]*

### Pump:
* Manufacturer: *[e.g. Medtrum]*
* Model: *[e.g. 200u or 300u]*

### OSAID app:
* App: *[e.g. Trio, Loop, iAPS]*
* Version Number: *[e.g. 0.7.0]*
* Repo: *nightscout/trio*
* Git Reference: *[e.g. commit hash]*

## Technical Details
*If applicable, provide any technical details that might help in diagnosing the problem. This could include logs, error messages, or relevant configuration details.*

## Additional context
*Add any other context about the problem here.*